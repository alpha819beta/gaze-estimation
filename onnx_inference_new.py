...
class GazeEstimationONNX:
    """
    Gaze estimation using ONNXRuntime with batch processing support (logits to radian decoded).
    Batch processing significantly improves GPU utilization and throughput.
    """
    ...
    def estimate(self, face_image: np.ndarray) -> Tuple[float, float]:
        ...
        input_tensor = np.expand_dims(self.preprocess(face_image), axis=0)
...
if __name__ == "__main__":
    ...
    while cap.isOpened():
        ...
        # Extract valid face crops and their bounding boxes
        face_crops = []
        valid_bboxes = []
        ...
        # Batch process all faces at once (much faster on GPU!)
        if face_crops:
            results = engine.estimate_batch(face_crops)
            
            for bbox, (pitch, yaw) in zip(valid_bboxes, results):
